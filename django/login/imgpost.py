import torch
import json
from login.ultralytics.yolo.engine.results import Results
from login.ultralytics.yolo.utils.plotting import save_one_box
from login.ultralytics.nn.autobackend import AutoBackend
from login.ultralytics.yolo.cfg import get_cfg
from login.ultralytics.yolo.data import load_inference_source
from login.ultralytics.yolo.data.augment import classify_transforms
from login.ultralytics.yolo.utils import DEFAULT_CFG, LOGGER, SETTINGS, callbacks, colorstr, ops
from login.ultralytics.yolo.utils.checks import check_imgsz, check_imshow
from login.ultralytics.yolo.utils.files import increment_path
from login.ultralytics.yolo.utils.torch_utils import select_device, smart_inference_mode
...
def posttext(request):
    ...
    # print(json_param)
    #提取图片数据
    img = json_param['data']['img']
    # print(img)

    model = 'login/pig.pt'
    source = base64_to_image(img)
    ...
    detection_img = image_to_base64(img)

    results={}
    results["img"] = detection_img
    results["list"] = l1
    print("尝试打开"+l1)
